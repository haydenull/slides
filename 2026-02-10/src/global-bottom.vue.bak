...
const opacity = computed<number>(() => +(frontmatter.value.growOpacity || 0.2))
...
const poly2 = usePloy(4)
const poly3 = usePloy(3)
</script>

<!-- 00dcc2  2366a6 5523a6 -->
<template>
  <div
  ...
  >
    <div
      class="clip bg-gradient-to-r from-[#badaff] to-white/10"
      :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
    />
    <div
      class="clip bg-gradient-to-l from-[#fde3ff] to-white/10"
      :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
    />
    <div
      class="clip bg-gradient-to-t from-[#e349ed] to-white/10"
      :style="{ 'clip-path': `polygon(${poly3})`, 'opacity': 0.1 }"
    />
  </div>
</template>
